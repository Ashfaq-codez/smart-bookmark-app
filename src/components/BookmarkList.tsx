...
export default function BookmarkList({ initialBookmarks }: { initialBookmarks: Bookmark[] }) {
  ...
  const saveEdit = async (id: number) => {
    ...
    if (error) alert(error.message)
    else setEditingId(null) // Close edit mode on success
  }
      ...
      {bookmarks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {bookmarks.map((bookmark, index) => {
            ...
            return (
              <div key={bookmark.id} className={`flex flex-col justify-between p-6 rounded-2xl border-4 border-gray-900 shadow-[6px_6px_0px_0px_rgba(17,24,39,1)] transition-all duration-200 ${!isEditing ? 'hover:translate-x-1 hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(17,24,39,1)]' : ''} ${cardColorClass}`}>
                
                {/* Header Actions */}
                <div className="flex justify-between items-start mb-6">
                  <div className="p-2 bg-white border-2 border-gray-900 rounded-xl shadow-[2px_2px_0px_0px_rgba(17,24,39,1)]">
                    <img src={`https://www.google.com/s2/favicons?domain=${domain}&sz=128`} alt={`${domain} logo`} className="w-8 h-8 object-contain" onError={(e) => {(e.target as HTMLImageElement).src = 'https://www.google.com/s2/favicons?domain=example.com&sz=128'}} />
                  </div>
                  
                  <div className="flex gap-2">
                    {isEditing ? (
                      <>
                        <button onClick={() => saveEdit(bookmark.id)} className="p-2 text-white bg-green-500 hover:bg-green-600 border-2 border-gray-900 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] active:translate-y-0.5 active:translate-x-0.5 active:shadow-none" title="Save"><CheckIcon /></button>
                        <button onClick={() => setEditingId(null)} className="p-2 text-gray-900 bg-white hover:bg-gray-100 border-2 border-gray-900 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] active:translate-y-0.5 active:translate-x-0.5 active:shadow-none" title="Cancel"><XIcon /></button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => startEditing(bookmark)} className="p-2 text-gray-900 bg-white hover:bg-yellow-100 border-2 border-gray-900 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] active:translate-y-0.5 active:translate-x-0.5 active:shadow-none" title="Edit"><EditIcon /></button>
                        <button onClick={() => deleteBookmark(bookmark.id)} className="p-2 text-white bg-red-500 hover:bg-red-600 border-2 border-gray-900 rounded-xl transition-all shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] active:translate-y-0.5 active:translate-x-0.5 active:shadow-none" title="Delete"><TrashIcon /></button>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="mb-4">
                  {isEditing ? (
                    ...
                    <>
                      <h4 className="text-xl font-black text-gray-900 truncate mb-1" title={bookmark.title}>{bookmark.title}</h4>
                      <p className="text-sm font-bold text-gray-600 truncate" title={bookmark.url}>{domain}</p>
                    </>
                  )}
                </div>

                {/* Bottom Link (Hidden while editing) */}
                {!isEditing && (
                  <div className="pt-4 border-t-4 border-gray-900/10 mt-2">
                    <a href={bookmark.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-black text-gray-900 hover:text-blue-600 uppercase tracking-widest transition-colors">Visit<ExternalLinkIcon /></a>
                  </div>
                )}
              </div>
            )
