...
const PlusIcon = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M12 5v14M5 12h14"/></svg>
...
export default function Sidebar(props: SidebarProps) {
  ...
  return (
    <aside className="w-full h-full flex flex-col p-6 overflow-y-auto bg-white dark:bg-[#0c0c0c] transition-colors [&::-webkit-scrollbar]:w-0">
      <div className="flex flex-col space-y-6">
        
        {/* TOP HEADER WITH INLINE ADD FOLDER BUTTON */}
        <div className="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className={`p-1.5 rounded-lg transition-colors cursor-pointer ${props.isAddingCategory ? 'bg-fuchsia-100 text-fuchsia-600 dark:bg-fuchsia-900/30 dark:text-fuchsia-400' : 'text-gray-400 hover:text-gray-800 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800'}`}
              title="Add New Folder"
            >
              <PlusIcon />
            </button>
            <button onClick={() => props.setIsMobileMenuOpen(false)} className="md:hidden p-1.5 text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-800 rounded-full">
              <SmallXIcon />
            </button>
          </div>
        </div>
        ...
        {props.isAddingCategory && (
          <div className="animate-in fade-in slide-in-from-top-2 duration-200">
            <input autoFocus type="text" placeholder="FOLDER NAME..." value={props.newCategoryName} onChange={(e) => props.setNewCategoryName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && props.handleAddCategory()} className="w-full px-5 py-4 text-sm font-black uppercase tracking-widest bg-gray-100 dark:bg-[#1a1a1a] text-gray-900 dark:text-white rounded-2xl outline-none focus:ring-2 focus:ring-fuchsia-500" />
          </div>
        )}
            ...
            return (
              <div key={parentFolder} className="flex flex-col gap-2">
                <div
                  ...
                  className={`group flex items-center justify-between px-5 py-4 rounded-3xl cursor-pointer transition-all ${isParentActive && !props.activeSubFilter ? 'bg-gradient-to-r from-fuchsia-600 to-orange-500 text-white shadow-xl shadow-fuchsia-500/20' : 'bg-gray-50 dark:bg-[#111] text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-[#1a1a1a]'}`}
                >
                ...
                {isExpanded && (
                  <div className="ml-8 pl-5 border-l-2 border-gray-200 dark:border-gray-800 flex flex-col gap-2 py-2">
                    {subfolders.map(sub => {
                    ...
                    {props.creatingSubFor === parentFolder ? (
                      <div className="flex gap-2 mt-2">
                        <input autoFocus type="text" placeholder="Name..." value={props.newSubfolderName} onChange={(e) => props.setNewSubfolderName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && props.handleAddSubfolder(parentFolder)} className="w-full px-4 py-3 text-sm font-bold uppercase rounded-xl outline-none bg-gray-100 dark:bg-[#1a1a1a] text-gray-900 dark:text-white focus:ring-2 focus:ring-fuchsia-500" />
      ...
      <div className="md:hidden mt-auto pt-10 flex flex-col gap-4 pb-12">
