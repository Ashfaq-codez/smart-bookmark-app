...
export default function BookmarkCard({
  ...
  return (
    <div
      ...
      className={`relative group flex flex-col bg-white dark:bg-gray-800 border-2 border-gray-900 rounded-2xl overflow-hidden shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 transition-all cursor-grab active:cursor-grabbing ${isDragged ? 'opacity-50 scale-95' : ''}`}
    >
      <button
      ...
      <div className="p-3 flex flex-col min-h-[95px] relative">
        ...
        <div className="absolute right-3 top-3 flex gap-2 opacity-100 md:opacity-0 mb-5 md:group-hover:opacity-100 transition-opacity z-10">
          <button 
            onClick={() => { setIsMoving(true); setIsEditing(false); }} 
            className="p-1.5 bg-yellow-100 dark:bg-gray-700 text-yellow-700 dark:text-yellow-400 border border-gray-900 rounded-md hover:bg-yellow-200 dark:hover:bg-gray-600 transition-colors cursor-pointer" title="Move to Folder"
          >
            <MoveIcon />
          </button>
          <button 
            onClick={() => { setIsEditing(true); setIsMoving(false); }} 
            className="p-1.5 bg-cyan-100 dark:bg-gray-700 text-cyan-700 dark:text-cyan-400 border border-gray-900 rounded-md hover:bg-cyan-200 dark:hover:bg-gray-600 transition-colors cursor-pointer" title="Edit"
          >
            <EditIcon />
          </button>
          <button 
            onClick={handleDelete} 
            className="p-1.5 bg-pink-100 dark:bg-gray-700 text-pink-700 dark:text-pink-400 border border-gray-900 rounded-md hover:bg-pink-200 dark:hover:bg-gray-600 transition-colors cursor-pointer" title="Delete"
          >
            <TrashIcon />
          </button>
        </div>
        ...
        (
          <div className="space-y-2 w-full mt-1 flex flex-col">
            <input type="text" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="w-full px-2 py-1.5 text-sm border-2 border-gray-900 rounded bg-white dark:bg-gray-700 dark:text-white outline-none" placeholder="Title" />
            <input type="url" value={editUrl} onChange={(e) => setEditUrl(e.target.value)} className="w-full px-2 py-1.5 text-[10px] border-2 border-gray-900 rounded bg-white dark:bg-gray-700 dark:text-white outline-none" placeholder="URL" />
            <div className="flex gap-2">
              <input type="text" list="category-options" value={editCategory} onChange={(e) => setEditCategory(e.target.value)} className="w-full px-2 py-1.5 text-[10px] border-2 border-gray-900 rounded bg-white dark:bg-gray-700 dark:text-white outline-none" placeholder="Folder" />
            ...
            <div className="flex gap-2 mt-1">
              ...
              <button onClick={() => setIsEditing(false)} className="flex-1 py-1.5 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-100 text-xs font-bold border-2 border-gray-900 rounded cursor-pointer">Cancel</button>
            </div>
          </div>
        ) : isMoving ?
        (
          <div className="space-y-3 w-full mt-1 flex flex-col flex-1 justify-center">
            <p className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-5">Move to Folder</p>
            <input type="text" list="category-options" value={moveCategory} onChange={(e) => setMoveCategory(e.target.value)} className="w-full px-2 py-2 text-xs border-2 border-gray-900 rounded bg-white dark:bg-gray-700 dark:text-white outline-none" placeholder="Main Folder" />
            ...
            <div className="flex gap-2 mt-auto pt-2">
              ...
              <button onClick={() => setIsMoving(false)} className="flex-1 py-1.5 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-100 text-xs font-bold border-2 border-gray-900 rounded cursor-pointer">Cancel</button>
            </div>
          </div>
          ...
          <>
            <a href={bookmark.url} target="_blank" rel="noopener noreferrer" className="block max-w-[75%] cursor-pointer">
              <h3 className="font-medium text-sm text-gray-900 dark:text-white truncate leading-tight hover:underline">{bookmark.title}</h3>
            </a>
